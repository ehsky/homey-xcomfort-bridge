...
class XComfortConnection {
  constructor(bridgeIp, authKey) {
    ...
    this.awaitingRenewedTokenApply = false;
        ...
        this.ws.on('close', (code, reason) => {
          ...
          if (this.connectionEstablished && !this.reconnecting) {
            ...
            setTimeout(() => {
              ...
              this.awaitingRenewedTokenApply = false; // Reset token renewal state
  ...
  async handleEncryptedMessage(msg) {
    // Send ACK for messages with 'mc' field
    if ('mc' in msg) {
      const ackMsg = {
    ...
    if (msg.type_int === MESSAGE_TYPES.SECRET_EXCHANGE_ACK) {
      ...
      // Received renewed token
      this.token = msg.payload.token;
      console.log('[XComfort] Token renewed successfully');
      
      // Apply the renewed token
      const applyRenewedMsg = {
        type_int: MESSAGE_TYPES.TOKEN_APPLY,
        mc: this.nextMc(),
        payload: { token: this.token },
      };
      this.sendEncrypted(applyRenewedMsg);
      this.awaitingRenewedTokenApply = true;
      
    } else if (msg.type_int === MESSAGE_TYPES.TOKEN_APPLY_ACK) {
      if (this.awaitingRenewedTokenApply) {
        // This is the ACK for the renewed token - now we're fully authenticated
        this.awaitingRenewedTokenApply = false;
        console.log('[XComfort] Renewed token applied, fully authenticated!');
        ...
        // First TOKEN_APPLY_ACK - need to renew the token
        console.log('[XComfort] Token applied, requesting token renewal...');
  ...
  processHomeData(payload) {
    console.log('[XComfort] Processing home data:', JSON.stringify(payload, null, 2));
        ...
        if (item.deviceId) {
          ...
          // Process known metadata info types (temperature, humidity ONLY)
            ...
            if (Object.keys(metadata).length > 0) {
              console.log(`[XComfort] Device ${item.deviceId} metadata:`, metadata);
