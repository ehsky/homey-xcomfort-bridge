...
class XComfortConnection {
  ...
  // Helper: Pad message with null bytes for AES block size (16)
  padToBlockSize(str) {
  ...
  async handleUnencryptedMessage(msg) {
    if (msg.type_int === 10) {
    ...
    } else if (msg.type_int === 12) {
      const scInitMsg = { type_int: 14, mc: this.nextMc(), payload: {} };
      this.ws.send(JSON.stringify(scInitMsg));
      console.log('[XComfort] Sent SC_INIT');
    } else if (msg.type_int === 14) {
      const pubkeyReq = { type_int: 14, mc: this.nextMc() };
      this.ws.send(JSON.stringify(pubkeyReq));
      console.log('[XComfort] Requested public key');
    } else if (msg.type_int === 15) {
      ...
      const encrypted = this.publicKey.encrypt(secretStr, 'RSAES-PKCS1-V1_5');
  ...
  async handleEncryptedMessage(msg) {
    // Send ACK for messages with mc field - based on xcomfort-python implementation
    // Reference: https://github.com/jankrib/xcomfort-python/blob/main/xcomfort/connection.py#L195-L197
    if ('mc' in msg) {
      const ackMsg = {
        type_int: 1,
        ref: msg.mc
    ...
    } else if (msg.type_int === 34) {
      console.log('[XComfort] Token applied, authenticated!');
      this.connectionState = 'connected';
      
      // Request initial data
      this.sendEncrypted({ type_int: 240, mc: this.nextMc(), payload: {} });
      this.sendEncrypted({ type_int: 242, mc: this.nextMc(), payload: {} });
      this.sendEncrypted({ type_int: 2, mc: this.nextMc(), payload: {} });
      
      // Start heartbeat
      this.startHeartbeat();
    } else if (msg.type_int === 300) {
      console.log('[XComfort] Received SET_ALL_DATA');
      this.processDeviceData(msg.payload);
    } else if (msg.type_int === 310) {
  ...
  async setDimmerValue(deviceId, dimmValue) {
    ...
    dimmValue = Math.max(1, Math.min(99, dimmValue));
  ...
  async requestDeviceStates() {
    ...
    try {
      await this.sendEncrypted({ type_int: 240, mc: this.nextMc(), payload: {} });
      await this.sendEncrypted({ type_int: 242, mc: this.nextMc(), payload: {} });
      await this.sendEncrypted({ type_int: 2, mc: this.nextMc(), payload: {} });
